...
# in usual packages
sa1<-rjdhf::stl(y, period=7, multiplicative = TRUE, swindow=53, twindow=9)

# Apply then multiplicative STL for annual (approximate) periodicity on the "seasonally adjusted" series
sa2<-rjdhf::stl(sa1$decomposition$sa, period=365, multiplicative = T, swindow=5)
...
sa5<-rjdhf::fractionalAirlineDecomposition(log(y), period=7)
sa6<-rjdhf::fractionalAirlineDecomposition(sa5$decomposition$sa, period=365.25)
